;;PLEUROBRANCHAEA BEHAVIOR SIMULATION v1.5
...
1113
588
Set length of film (# ticks) and the frames/sec (more frames = more MB).
9
116.0
1
...
1001
346
Average satiation for all slugs, at the point when a food-item is consumed.
9
0.0
1
...
1168
397
Average satiation for all slugs, at the point when red-odored food-items are consumed.
9
15.0
1
...
FILM LENGTH: Type in the number of ticks that you would like to record the simulation for. (Note: at about 15 frames/sec, the movie will run to 0.5 MB per tick, which can lead to quite a large file. Please make sure you have enough space to save the file. After saving a movie, it is advised to use video-editting software to decrease the resolution and file size.)

FRAMES/SEC: Adjust the slider to adjust the frames/second rate of the film.

RECORD: Press this button, and follow the instructions on the subsequent pop-ups, to save a .mov movie file to your computer of the simulation (including any user-interactions).
...
-Try changing the hermisenda/ flabellina/ faux-hermisenda population levels. Does including faux-hermisenda make it more or less likely that a red-odored individual gets bitten?
